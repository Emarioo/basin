...
typedef struct COFF_File_Header {
    ...
    u32 NumberOfSymbols;
    u16 SizeOfOptionalHeader; // always zero for object files (not executables)
...
typedef struct Section_Header {
    ...
    u32 VirtualSize; // 0 for object files
    u32 VirtualAddress; // should be 0 for simplicity, it offsets relocations but isn't necessary.
    u32 SizeOfRawData; // size of section data
    u32 PointerToRawData; // 4-byte aligned for best performance
...
typedef u8 Type_Representation_MSB;
